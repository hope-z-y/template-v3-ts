...
const authenticate = async (page: Page) => {
  ...
  await page.route("**/proxy-api/auth/refresh", (route) =>
    route.fulfill(respond({ accessToken: "access", refreshToken: "refresh", expiresIn: 3600 })),
  );
  await page.route("**/proxy-api/auth/profile", (route) => {
    if (route.request().method() === "PATCH") return route.fulfill(respond(profile.userInfo));
    return route.fulfill(respond(profile));
  });
  await page.route("**/proxy-api/auth/public-key", (route) => route.fulfill(respond(publicKey)));
  await page.route("**/proxy-api/auth/verify-password", (route) => {
    verifyAttempts += 1;
    if (verifyAttempts === 1) {
  ...
  await page.route("**/proxy-api/system/department/tree", (route) => route.fulfill(respond([])));
  await page.route("**/proxy-api/system/user**", (route) =>
    route.fulfill(
      respond({
        rows: [
          {
