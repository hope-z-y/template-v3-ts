...
import type { Status } from "@/utils/index.ts";
// #endregion
...
const editingRecord = ref<IUserRow | null>(null);
...
const columns = computed<DataTableColumns<IUserRow>>(() => {
  const visibleKeys = new Set(
    columnOptions.value.filter((item: PageColumnOption) => item.visible).map((item: PageColumnOption) => item.key),
  );
