    ...
    <Teleport to="body">
        <Transition enter-active-class="transition-opacity duration-200 ease-out" enter-from-class="opacity-0"
            ...
            <div v-if="visible"
                ...
                <Transition appear enter-active-class="transition-all duration-200 ease-out"
                    enter-from-class="opacity-0 -translate-y-2 scale-[0.98]"
                    enter-to-class="opacity-100 translate-y-0 scale-100"
                    leave-active-class="transition-all duration-200 ease-in"
                    leave-from-class="opacity-100 translate-y-0 scale-100"
                    leave-to-class="opacity-0 -translate-y-2 scale-[0.98]">
                    <div v-if="visible" class="h-fit w-full max-w-xl overflow-hidden rounded-xl border shadow-2xl"
                        :style="modalStyle" @click.stop>
                        <div class="flex items-center gap-3 border-b border-black/6 px-4 py-3 dark:border-white/8">
                            ...
                            <input ref="modalInputRef" v-model="keyword" type="search"
                                ...
                                @keydown.enter.prevent="selectActive" @keydown.esc.prevent="closeSearch" />
                            <button type="button"
                                ...
                                aria-label="关闭搜索" @click="closeSearch">
                                <NIcon :size="18">
                                    <Dismiss24Regular />
                                </NIcon>
                            </button>
                        ...
                        <div class="max-h-[min(50vh,360px)] overflow-y-auto p-2">
                            ...
                            <ul v-else class="m-0 list-none p-0">
                                <li v-for="(item, index) in filteredItems" :key="item.id"
                                    class="cursor-pointer rounded-lg px-3 py-2.5 transition-colors" :class="index === activeIndex
                                        ? ''
                                        : 'text-black/78 hover:bg-black/4 dark:text-white/78 dark:hover:bg-white/6'"
                                    :style="index === activeIndex ? activeItemStyle : undefined"
                                    @mouseenter="activeIndex = index" @mousedown.prevent @click="handleSelect(item)">
